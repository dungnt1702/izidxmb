...
<input type="submit" class="btn-sm btn btn-primary check-check-all" name="allow_all" value="Duyệt nhiều" onclick="return confirm('Bạn có chắc chắn muốn đồng ý các phiếu đánh giá này?')" />
    ...
    @foreach ($a_checkpoints as $key => $o_checkpoint)
        ...
        <tr>
            ...
            <td>    {{ isset($a_Users[$o_checkpoint->user_id]['code']) ? $a_Users[$o_checkpoint->user_id]['code'] : '' }}</td>
            <td>    {{ isset($a_Users[$o_checkpoint->user_id]['name']) ? $a_Users[$o_checkpoint->user_id]['name'] : '' }}</td>
            <td>    {{ isset($a_Users[$o_checkpoint->user_id]['email']) ? $a_Users[$o_checkpoint->user_id]['email'] : '' }}</td>
            <td>    {{ isset($a_Users[$o_checkpoint->user_id]['position']) ? $a_Users[$o_checkpoint->user_id]['position'] : '' }}</td>

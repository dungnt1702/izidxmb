...
<form method="get" action="" id="frmFilter" name="frmFilter"  class="form-inline">
    ...
    @foreach ($a_leave_types as $key => $o_leave_types)
        ...
        <tr>
            ...
            <td>    {{ $o_leave_types->created_at }}</td>
            <td>    {{ $o_leave_types->updated_at }}</td>
            <td>                    
                <?php
                ...
                <a id="trash_switch_" href="javascript:GLOBAL_JS.v_fDelRow({{ $o_leave_types->id }},1)" title="Cho vào thùng rác" class="not-underline">
                <i class="fa fa-trash fa-fw text-danger"></i>
                </a>
                <?php }else{ ?>
                <a title="Khôi phục user" href="javascript:GLOBAL_JS.v_fRecoverRow({{ $o_leave_types->id }})"  title="Edit" class="not-underline">
                    <i class="fa fa-upload fw"></i>
                </a>
                <a id="trash_switch_" href="javascript:GLOBAL_JS.v_fDelRow({{ $o_leave_types->id }},2)" title="Xóa vĩnh viễn" class="not-underline">

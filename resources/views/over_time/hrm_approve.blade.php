...
@section('content')
    ...
    <form method="POST" action="">
        <input type="hidden" name="_token" value="{!! csrf_token() !!}">
        <h3 class="col-xs-12 no-padding">Danh sách đăng ký làm thêm</h3>
                ...
                @foreach ($a_data as $a_val)
                    ...
                    <tr id="tr_<?php echo $a_val->id ?>" class="alert {{ $sz_gbcolor }}">
                        ...
                        <td>
                            ...
                            <button type="button" class="btn-sm btn btn-primary hrm-allow-ot" data-toggle="modal" data-target="#allow" id="confirm_<?php echo $a_val->id ?>">Đồng ý</button>
                            <button type="button" class="btn-sm btn btn-danger hrm-reject-ot" data-toggle="modal" data-target="#reject" id="confirm_<?php echo $a_val->id ?>">Từ chối</button>
    ...
    <script>
        var a_SendMultiEmailOT = <?php echo json_encode(session('sendMutiMailOT'))?>;
    </script>

    ...
    <table class="table table-responsive table-hover table-striped table-bordered">
        ...
        @foreach ($a_user as $key => $user)
            ...
            <tr tr_<?php echo $user->id ?>>
                ...
                <td>    {{ $user->name }}</td>
                <td>    {{ $user->email }}</td>
                <td>    {{ $user->old_department_name }}</td>
                <td>    {{ $user->old_manager_name }}</td>
                <td>    {{ $user->new_department_name }}</td>
                <td>    {{ $user->new_manager_name }}</td>
                <td>    {{ $user->user_comment }}</td>
                <td>    {{ $user->created_at }}</td>
                <td>
                    <?php if($user->status == 0) { ?>
                    <button type="button" class="btn-sm btn btn-primary allow-change-manager {{ $sz_disable }}" <?php if($sz_disable == 'active'){ ?> data-toggle="modal" data-target="#allow" <?php }?> id="confirm_<?php echo $user->id ?>">Đồng ý</button>
                    <button type="button" class="btn-sm btn btn-danger reject-change-manager {{ $sz_disable }}" <?php if($sz_disable == 'active'){ ?> data-toggle="modal" data-target="#reject" <?php }?> id="confirm_<?php echo $user->id ?>">Từ chối</button>

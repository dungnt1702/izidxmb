...
<?php $__env->startSection('content'); ?>
    <h3 class="col-xs-12 no-padding">Bảng chấm công</h3>
    <form method="get" action="" id="frmFilter" name="frmFilter"  class="form-inline">
        ...
        <div class="form-group">
            ...
            <a class="btn btn-primary btn-sm" href="<?php echo '/time_sheet/merge_timesheet?month='.$a_Data['a_search']['search_month'].'&year='.$a_Data['a_search']['search_year']?>" target="_blank">Merge chấm công</a>
    ...
    <table class="table table-responsive table-hover table-striped table-bordered">
        ...
        ?>
            <tr>
                ...
                <?php foreach ($a_Data['a_RangeDate'] as $i_Date) { ?>
                <td class="td_report">
                    <?php if(is_array($o_Timesheet->$i_Date))
                    {
                        $a_TimeCheck = $o_Timesheet->$i_Date;
                        echo '<b>Giờ vào: </b>' . $a_TimeCheck[0] . '<br>';
                        echo '<b>Giờ ra: </b>' . $a_TimeCheck[1];
                    }
                    ?>
                </td>
                <?php } ?>
